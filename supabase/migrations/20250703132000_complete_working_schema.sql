-- =====================================================
-- Rexera 2.0 Complete Working Schema
-- Complete database schema that combines essential features
-- =====================================================
...
CREATE TABLE tasks (
    ...
    CONSTRAINT check_hil_assignment CHECK (
        ...
        (executor_type = 'HIL' AND assigned_to IS NOT NULL)
    )
);
...
CREATE POLICY "Enable all access for authenticated users" ON agent_executions FOR ALL USING (auth.role() = 'authenticated');
